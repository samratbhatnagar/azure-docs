...
This article shows Contoso are rehosting their on-premises two-tier Linux Apache MySQL PHP (LAMP)service desk app (osTicket) by migrating it to Azure, using the Azure Site Recovery service. If you'd like to try out the open source app used in this article, you can download it from [github](https://github.com/osTicket/osTicket).
...
Migrate a Linux app to Azure VMs (this article) | Rehost (lift-and-shift) | In this article, Contoso rehosts a two-tier Linux app using Azure Site Recovery. 
...
- Contoso wants to migrate their two-tier on-premises service desk app.
...
2. They import the template into VMware to create the VM, and deploy the VM.
    ...
    ![OVF template](./media/ccontoso-migration-rehost-linux-vm/vcenter-wizard.png)
...
![Azure guest agent](./media/contoso-migration-rehost-linux-vm/azure-guest-agent.png)
...
In this scenario we showed the fourth scenario in the Contoso rehost (lift-and-shift) migration series. We migrated an on-premises service desk app tiered over two Linux VMs to Azure.

In the fifth article in the series, we'll show how Contoso migrate the same service desk app to Azure, using Site Recovery to migrate the frontend VM for the app, and migrating the app database using backup and restore to Azure Database for MySQL, using the MySQL workbench tool.
